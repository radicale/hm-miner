...
if [ "$VARIANT" = "COMP-PISCESP100" ]; then
  wget -O - "${OVERRIDE_CONFIG_URL}" | \
    sed s/i2c-1/i2c-0/ > "/opt/miner/releases/$HELIUM_GA_RELEASE/sys.config"
else
  wget -O "/opt/miner/releases/$HELIUM_GA_RELEASE/sys.config" "${OVERRIDE_CONFIG_URL}"
fi
